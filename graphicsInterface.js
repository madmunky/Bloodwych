...
function drawPocketUI(p) {
    ...
    for (y = 0; y < 2; y++) {
        for (x = 0; x < 6; x++) {
            ...
            if (champion[p.champion[p.uiRightPanel.activePocket]].pocket[i] === 0) {
                ...
                pocketImg = UI_GFX_ICON_BOOKOFSKULLS;
    ...
    for (x = 0; x < 6; x++) {
        ...
        switch (x) {
            ...
            case 5:
                {
                    ctx.drawImage(gfxUI[UI_GFX_ICON_BACK], ((p.ScreenX + 225) + (x * 16)) * scale, ((p.ScreenY + 63)) * scale, gfxUI[UI_GFX_POCKET_EMPTY].width * scale, gfxUI[UI_GFX_POCKET_EMPTY].height * scale);
                };
                break
